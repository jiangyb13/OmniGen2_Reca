...
def load_chat_pipeline(args: argparse.Namespace, accelerator: Accelerator, weight_dtype: torch.dtype) -> OmniGen2ChatPipeline:
...
def load_gen_pipeline(args: argparse.Namespace, accelerator: Accelerator, weight_dtype: torch.dtype) -> OmniGen2Pipeline:
...
def main(args: argparse.Namespace, root_dir: str) -> None:
    ...
    chat_pipeline = load_chat_pipeline(args, accelerator, weight_dtype)
    # gen_pipeline = load_gen_pipeline(args, accelerator, weight_dtype)

    input_images = preprocess(args.input_image_path)

    # Generate and save image
    results = run(args, accelerator, chat_pipeline, args.instruction, args.negative_prompt, input_images)
