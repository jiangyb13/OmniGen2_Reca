...
from longclip_b.model import longclip
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scheduler",
        type=str,
        default="euler_custom",
        choices=["euler", "dpmsolver++", "euler_custom"],
...
def load_pipeline(args: argparse.Namespace, accelerator: Accelerator, weight_dtype: torch.dtype) -> OmniGen2ChatPipeline:
    ...
    elif args.scheduler == "euler_custom":
        # [注意] 请根据你实际存放 FlowMatchEulerDiscreteSchedulerCustom 的位置修改 import 路径
        # 例如，如果你把它放在了 pipeline 文件旁边：
        # from omnigen2.pipelines.omnigen2.scheduler_custom import FlowMatchEulerDiscreteSchedulerCustom
        # 或者如果你把它放在了 schedulers 目录下：
        # from omnigen2.pipelines.omnigen2.Scheduler import FlowMatchEulerDiscreteSchedulerCustom
        
        # # 使用原有 scheduler 的配置来初始化自定义 scheduler
        # # 这样可以继承 num_train_timesteps 等参数
        # scheduler = FlowMatchEulerDiscreteSchedulerCustom.from_config(pipeline.scheduler.config)
...
def main(args: argparse.Namespace, root_dir: str) -> None:
    ...
    pipeline.clip_model, pipeline.clip_processor = longclip.load("./pretrained_models/LongCLIP-B/longclip-B.pt", device=device)
    pipeline.tokenize = longclip.tokenize
