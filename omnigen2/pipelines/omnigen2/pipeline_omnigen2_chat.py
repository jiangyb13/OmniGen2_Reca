...
from .Scheduler import FlowMatchEulerDiscreteSchedulerCustom
...
class OmniGen2ChatPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        vae: AutoencoderKL,
        scheduler: FlowMatchEulerDiscreteSchedulerCustom,
    ...
    def _apply_chat_template(self, prompt: str, images: List = None, llm_und=False):
    ...
    def generate_image(
        ...
        image = self.processing2(
            ...
            re_update_num=re_update_num,
            update_lr=update_lr,
            use_save_pic=use_save_pic,
    ...
    def __call__(
        ...
        # with torch.no_grad():
        #     prompt = self._apply_chat_template(prompt, input_images, True)
    ...
    def processing2(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                loss_list = []
                lookback_clip_list = []
                lookback_latents_list = []
                for re_update in range(re_update_num_ + 1):
                    final_flag = False if re_update_num_ != 0 and update_flag and re_update != re_update_num else True
                    # breakpoint() 
                    ...
                    prev_sample, prev_original_sample = self.scheduler.step(model_pred, t, latents, return_dict=False)

                    if update_flag and not final_flag:
                        loss, vlm_output = self.RecALoss(prev_original_sample, original_prompt, image_shapes=latents.shape)
                        loss_list.append(loss.item())
                        print(vlm_output)

                        with torch.no_grad():
                            if use_lookback:
                                origin_t_step = self.scheduler._step_index
                                latents_copy = prev_sample.detach()
                                latents_copy_prev = prev_original_sample.detach()
                                from tqdm import tqdm
                                for idx in tqdm(range(i+1, lookback_steps)):
                                    pred = self.predict(
                                        t=timesteps[idx],
                                    ...
                                    pp, ppp = self.scheduler.step(pred, t, latents_copy, return_dict=False)
                                    latents_copy = pp
                                    latents_copy_prev = ppp
                                    # breakpoint()
                                self.scheduler._step_index = origin_t_step
                                lookback_clip_list.append(self.calc_clip(latents_copy_prev, original_prompt, image_shapes=latents.shape).item())
                                lookback_latents_list.append(prev_sample.detach().requires_grad_())
                                # breakpoint()
                    elif update_flag and final_flag:
                        with torch.no_grad():
                            loss, vlm_output = self.RecALoss(prev_original_sample, original_prompt, image_shapes=latents.shape)
                            loss_list.append(loss.item())
                            print(vlm_output)
                    ...
                    if not final_flag and update_flag:
                        # loss = torch.sum(prev_original_sample)
                        grad = torch.autograd.grad(loss, latents)[0]
                        if grad.norm(p=2) > 0.01:
                            grad = grad * (0.01 / grad.norm(p=2))
                        latents = latents - update_scale * grad
                        ...
                        if use_lookback and update_flag:
                            best_index = max(
                                range(len(lookback_clip_list)),
                                key=lambda i: (-lookback_clip_list[i], i)
                            )
                            latents = lookback_latents_list[best_index]
                            print(f"lookbac clip: {lookback_clip_list}")
                            print(f"best of index: {best_index}")
                            breakpoint()

                    if use_save_pic and update_flag:
                        with torch.no_grad():
                            lat = prev_original_sample
                            if self.vae.config.scaling_factor is not None:
                                lat = lat / self.vae.config.scaling_factor
                            if self.vae.config.shift_factor is not None:
                                lat = lat + self.vae.config.shift_factor
                            image = self.vae.decode(lat, return_dict=False)[0]
                            pred_image = self.image_processor.postprocess(image.detach(), output_type="pil")[0]
                            import os
                            os.makedirs("./debug/pic/", exist_ok=True)
                            pred_image.save(f"debug/pic/debug_x{i}_{re_update}_{loss.item()}_step.png")
        ...
        # breakpoint()
        # torch.save(latents, 'lat.pt')
        # x = torch.load('lat.pt')
        # print(torch.equal(x, latents))
        if enable_taylorseer:
        ...
        # breakpoint()
        # torch.save(latents, 'lat1.pt')
        # x = torch.load('lat1.pt')
        # print(torch.equal(x, latents))
        latents = latents.to(dtype=dtype)
        if self.vae.config.scaling_factor is not None:
            latents = latents / self.vae.config.scaling_factor
        if self.vae.config.shift_factor is not None:
            latents = latents + self.vae.config.shift_factor
        # breakpoint()
        # torch.save(latents, 'lat2.pt')
        # x = torch.load('lat2.pt')
        # print(torch.equal(x, latents))
        image = self.vae.decode(latents, return_dict=False)[0]
        
        return image
    
    def RecALoss(self, preview_img, prompt, image_shapes=None):

        decode_latents = preview_img.clone().to(self.vae.dtype)
        if self.vae.config.scaling_factor is not None:
        ...
        pred_image_tensor = self.vae.decode(decode_latents, return_dict=False)[0]
        # breakpoint()
        pred_image_pil = self.image_processor.postprocess(pred_image_tensor.detach(), output_type="pil")[0]
        
        with torch.no_grad():
            vlm_prompt = f"Based on the text prompt '{prompt}' and the provided noisy intermediate image, describe the envisioned perfect final image in one concise paragraph. Ignore noise and artifacts, focusing strictly on the visual details and atmosphere. If there is any conflict between the visual features and the text prompt, you must strictly follow the text prompt. Only one paragraph is required."
            vlm_prompt = self._apply_chat_template(vlm_prompt, [pred_image_pil], llm_und=True)
            vlm_output_text = self.generate_text(vlm_prompt, [pred_image_pil])[0]
        
        text_inputs = self.tokenize([vlm_output_text]).to("cuda")
        ...
        image_embeds = self.clip_model.encode_image(image_inputs)
    ...
    @torch.no_grad()
    def calc_clip(self, preview_img, prompt, image_shapes=None):
        decode_latents = preview_img.clone().to(self.vae.dtype)
        if self.vae.config.scaling_factor is not None:
            decode_latents = decode_latents / self.vae.config.scaling_factor
        if self.vae.config.shift_factor is not None:
        ...
        pred_image_tensor = self.vae.decode(decode_latents, return_dict=False)[0]
        pred_image_pil = self.image_processor.postprocess(pred_image_tensor.detach(), output_type="pil")[0]
        pred_image_pil.save("./debug/debug_middle.png")
